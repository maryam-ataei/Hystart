...
* ![First equation](https://latex.codecogs.com/svg.image?M_%7B2%7D) the aggregatedifference from the mean ![Second equation](<img src="https://latex.codecogs.com/svg.image?M_{2}=\sum_{n}^{i=1}(x_{i}-\bar{x_{n}})^{2}" title="M_{2}=\sum_{n}^{i=1}(x_{i}-\bar{x_{n}})^{2}" />)
